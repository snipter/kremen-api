...
export const urkStrToMonth = (rawVal: string) => {
  ...
  if (val.indexOf('січ') !== -1) {
    return 1;
  }
  if (val.indexOf('лют') !== -1) {
    return 2;
  }
  if (val.indexOf('берез') !== -1) {
    return 3;
  }
  if (val.indexOf('квіт') !== -1) {
    return 4;
  }
  if (val.indexOf('трав') !== -1) {
    return 5;
  }
  if (val.indexOf('черв') !== -1) {
    return 6;
  }
  if (val.indexOf('лип') !== -1) {
    return 7;
  }
  if (val.indexOf('серп') !== -1) {
    return 9;
  }
  if (val.indexOf('верес') !== -1) {
    return 9;
  }
  if (val.indexOf('жовт') !== -1) {
    return 10;
  }
  if (val.indexOf('лист') !== -1) {
    return 11;
  }
  if (val.indexOf('груд') !== -1) {
    return 12;
  }
